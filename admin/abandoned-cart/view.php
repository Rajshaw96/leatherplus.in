    ...
    if ($_SESSION['admin_role'] == "sales" || $_SESSION['admin_role'] == "admin" || $_SESSION['admin_role'] == "shopmanager") {
        ...
        if (isset($_GET['q'])) {
            ...
            if ($connStatus == true) {
                ...
                $result_orderdetails = $database->getData("SELECT * FROM `abandoned_orders` LEFT JOIN `users` ON `abandoned_orders`.`order_userid` = `users`.`user_id` WHERE `order_id` = '" . $_GET['q'] . "'");
            ...
            include('../../views/admin/abandoned-cart/view.inc.php');
        }
    } else {
