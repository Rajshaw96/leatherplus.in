...
if ($request->checkRequestHash($_POST['key'], $_SESSION['secretcode']) == true) {
    ...
    if (isset($_POST['attribute_name'])) {
        ...
        if ($connStatus == true) {
            ...
            $result = $database->runQuery("INSERT INTO `product_attributes`(`pattrib_name`, `pattrib_type`) VALUES('".$_POST['attribute_name']."', '".$_POST['attribute_type']."')");

            if($result == true){

                header('location:'. $url->baseUrl("admin/products/attributes?m=add1"));
            }
            else{
