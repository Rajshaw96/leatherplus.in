...
if (isset($_POST['contactemail'])) {
    ...
    if ($connStatus == true) {
        ...
        if ($email_notifs->sendEmail($admin_email, "Leather Plus", "1 New Quick Contact Message From Leather Plus Website", "Name: ".$_POST['contactname']."<br>"."Email: ".$_POST['contactemail']."<br>"."Message: ".$_POST['contactmessage']) == true) {

            header('location:' . $url->baseUrl($_POST['page'].'?m=qsend1'));
        } else {

            header('location:' . $url->baseUrl($_POST['page'].'?m=qsend0'));
        }
    } else {
        header('location:' . $url->baseUrl($_POST['page'].'?m=qsend0'));
    }
